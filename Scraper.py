...
for stat in roll_apply_b:
    roll = 'Roll_'+stat
    df_b_sorted[roll] = df_b_sorted.groupby('Name')[stat].rolling(10).mean().shift(1).reset_index(0,drop=True)

for stat in roll_apply_p:
    roll = 'Roll_'+stat
    df_p_sorted[roll] = df_p_sorted.groupby('Name')[stat].rolling(10).mean().shift(1).reset_index(0,drop=True)
